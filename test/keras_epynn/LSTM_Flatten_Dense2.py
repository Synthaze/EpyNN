"""
This will run layers = [input, LSTM] with LSTM(20) in many-to-one mode
...
# Layers
keras_LSTM = KLSTM(N_CELLS, kernel_initializer=initializer, return_sequences=True, batch_input_shape=(N_SAMPLES, LEN_SEQ, V_SIZE), activation='tanh')
keras_flatten = KFlatten()
keras_dense = KDense(OUTPUT_NODES, activation='sigmoid', kernel_initializer=initializer)

# Model
keras_model = Sequential()
keras_model.add(keras_LSTM)
...
keras_lstm = keras_model.layers[0]

U = keras_lstm.get_weights()[0]
W = keras_lstm.get_weights()[1]
b = keras_lstm.get_weights()[2]

shared_L_Ui = U[:, :keras_lstm.units]
shared_L_Uf = U[:, keras_lstm.units: keras_lstm.units * 2]
shared_L_Ug = U[:, keras_lstm.units * 2: keras_lstm.units * 3]
shared_L_Uo = U[:, keras_lstm.units * 3:]

shared_L_Wi = W[:, :keras_lstm.units]
shared_L_Wf = W[:, keras_lstm.units: keras_lstm.units * 2]
shared_L_Wg = W[:, keras_lstm.units * 2: keras_lstm.units * 3]
shared_L_Wo = W[:, keras_lstm.units * 3:]

shared_L_bi = b[:keras_lstm.units]
shared_L_bf = b[keras_lstm.units: keras_lstm.units * 2]
shared_L_bg = b[keras_lstm.units * 2: keras_lstm.units * 3]
shared_L_bo = b[keras_lstm.units * 3:]
...
print('shared_L_Ui.shape:', shared_L_Ui.shape)
print('shared_L_Uf.shape:', shared_L_Uf.shape)
print('shared_L_Ug.shape:', shared_L_Ug.shape)
print('shared_L_Uo.shape:', shared_L_Uo.shape)

print('shared_L_Wi.shape:', shared_L_Wi.shape)
print('shared_L_Wf.shape:', shared_L_Wf.shape)
print('shared_L_Wg.shape:', shared_L_Wg.shape)
print('shared_L_Wo.shape:', shared_L_Wo.shape)

print('shared_L_bi.shape:', shared_L_bi.shape)
print('shared_L_bf.shape:', shared_L_bf.shape)
print('shared_L_bg.shape:', shared_L_bg.shape)
print('shared_L_bo.shape:', shared_L_bo.shape)
...
from nnlibs.lstm.models import LSTM as ELSTM
...
epynn_LSTM = ELSTM(N_CELLS)
epynn_LSTM.p['Ui'] = shared_L_Ui
epynn_LSTM.p['Wi'] = shared_L_Wi
epynn_LSTM.p['bi'] = shared_L_bi

epynn_LSTM.p['Ug'] = shared_L_Ug
epynn_LSTM.p['Wg'] = shared_L_Wg
epynn_LSTM.p['bg'] = shared_L_bg

epynn_LSTM.p['Uf'] = shared_L_Uf
epynn_LSTM.p['Wf'] = shared_L_Wf
epynn_LSTM.p['bf'] = shared_L_bf

epynn_LSTM.p['Uo'] = shared_L_Uo
epynn_LSTM.p['Wo'] = shared_L_Wo
epynn_LSTM.p['bo'] = shared_L_bo
...
epynn_model = EpyNN(layers=[embedding, epynn_LSTM, flatten, epynn_dense], settings=settings, seed=SEED)
